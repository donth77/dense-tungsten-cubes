...
export class ScaleInstrument {
  ...
  beforePhysics(): void {
     ...
     */
    const maxDamp = (Math.abs(vy) * S.platterKg) / config.loop.DT;
  ...
  #load(): { kg: number; count: number; motionMps: number; grabbed: boolean; partial: boolean } {
    ...
    for (const e of this.ctx.entities.all) {
      ...
      // Its footprint runs off the platter, so something else is carrying part of it.
      if (dx + half > p.x || dz + half > p.z) out.partial = true;
    }
    return out;
  }
